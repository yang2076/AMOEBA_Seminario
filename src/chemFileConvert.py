...
#  PSI42XYZ:  From psi4 to the normal xyz file(SAPT calculation) 
...
def LOG2COM(LOG, counterpoise=False,optCartesian=False, nproc=8, QM='MP2', basis='6-31G*', extraKeyword="Guess=INDO MaxDisk=100GB freq", write_new_file = True):
  pTable = {'1':'H','2':'He','3':'Li','4':'Be','5':'B','6':'C','7':'N','8':'O',\
                 '9':'F','10':'Ne','11':'Na','12':'Mg','13':'Al','14':'Si','15':'P',\
                 '16':'S','17':'Cl','18':'Ar','19':'K','20':'Ca','22':'Ti','26':'Fe',\
                 '27':'Co','28':'Ni','29':'Cu','30':'Zn','35':'Br','53':'I'}
  data = readLOG(LOG)
  jobType = " opt=Tight "
  if optCartesian == True:
    jobType = " Opt=(tight,Cartesian) "
  ...
  if(fileName[-4:] == ".log"):
    fileName = LOG.split(".log")[0]

  comName = fileName+".com_1"
  if(not write_new_file):
    comName = fileName+".com"
  fout=open(comName, 'w')
    
  if counterpoise==False:
    fout.write("%chk="+"%s.chk\n"%(fileName))
    fout.write("%nproc="+str(nproc)+"\n%Mem=32GB\n#p "+QM+"/"+basis+extraKeyword+"%s\n\n%s \n\n%3s%3s\n"%(jobType,jobType,charge,multip))
...
def PSI42XYZ(PSI4):
  contents = []
  xyz = open(PSI4[:-5] + '.xyz', 'w')
  with open(PSI4,'r') as f_0:
    lines = f_0.readlines()
    num = 0
    count = 0
    for line in lines:
      num += 1
      if("--" in line or "0 1" in line or "-1 1" in line or "1 1" in line):
        continue
      elif("units" in line):
        break
      elif(num >= 7):
